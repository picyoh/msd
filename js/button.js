...
//Form
    ...
    //Boutons
        if (newQuestion != undefined) {
          ...
          //insertion div container input
                  var newInputDiv = document.createElement("div");
                  newInputDiv.className = "inputContainer";
            ...
            //insertion label input
                  var newLabel = document.createElement('label');
                  newLabel.className = "labelInputImg";
                  newLabel.htmlFor = newBtnContent[j].id;
                  newLabel.innerHTML = newBtnContent[j].id;
            ...
            //ProgressBar
                  ...
                  var progressPointValue = parseInt(newLineContent.value);

                  //calcul pourcenatge longueur vh
                  var prctWidth = 150 / pourPB;
                  var progressBarValue =  prctWidth * progressPointValue;

                  //curseur width root css
                  document.documentElement.style.setProperty('--cursor-width', prctWidth + "vh");
                  ...
                  if(newLineContent.value == 1) {
                    ...
                    divProgress.appendChild(progressWrap);

                    //creer ProgressBar
                    var progressBar = document.createElement('div');
                    progressBar.id = "progressBar";
                    progressBar.style = "width: " + prctWidth + "vh";
                    progressWrap.appendChild(progressBar);

                    //creer indicateur de position
                    var actualPosition = document.createElement("div");
                    actualPosition.id = "actualPosition";
                    actualPosition.style = "visibility: hidden;"
                    divProgress.appendChild(actualPosition);

                    //creer Liste
                    var progressList = document.createElement("ul");
                    progressList.id = "progressList";
                    divProgress.appendChild(progressList);

                    //creer points
                    for (var p = 1; p <= (numberQuestions + 1) ; p++) {
                      //creer li
                      var progressPointList = document.createElement("li");
                      progressPointList.id = "pp" + p;
                      progressPointList.className = "pplist";
                      progressList.appendChild(progressPointList);

                      //creer lien
                      var progressPointLink = document.createElement('a');
                      progressPointLink.className = "pplink";
                      progressPointLink.href = "#line " + p;
                      progressList.appendChild(progressPointLink);

                      //creer point
                      var progressPointImg = document.createElement('span');
                      progressPointImg.id = "point " + p;
                      progressPointImg.className = "ppimg";

                      // colorer premier point
                      if (p == 1){
                        progressPointImg.style = "background: rgba(220, 220, 220, 0.5);";
                      }
                      progressPointLink.appendChild(progressPointImg);

                      //position curseur
                      var pointPosition = document.getElementById("pp1").getBoundingClientRect().x;
                      document.getElementById("actualPosition").style = "left: " + (pointPosition - 2) + "px;";

                      }
                    }else{
                      //addition progressValue
                      document.getElementById("progressBar").style = "width: " + progressBarValue + "vh" ;

                      //colorer points
                      var actualPoint = document.getElementById("point " + progressPointValue);
                      document.getElementById("point " + progressPointValue).style = "background: rgba(220, 220, 220, 0.5);";

                      //position curseur actuelle
                      var pointPosition = document.getElementById("pp" + progressPointValue).getBoundingClientRect().x;
                      document.getElementById("actualPosition").style = "left: " + pointPosition + "px;";
                      }

                      //cacher div precedente
                      var prevDivValue = progressPointValue - 1;
                      var prevDiv = "line " + prevDivValue ;
                      document.getElementById(prevDiv).style = "visibility: hidden; width: 0;";
